...
| (next, urgent) chore HOTFIX — Attack avatar 變超大 fix (chore #177 hotfix). Pixi 8 `Sprite.width/.height` setter uses internal scale; subsequent `avatar.scale.set(s,s)` in Phase 1-5 overwrites size → sprite shows native texture pixels. Fix: wrap Sprite in Container, animate Container scale, keep Sprite size fixed at 120px | [`chore/attack-avatar-scale-overwrite-fix.md`](chore/attack-avatar-scale-overwrite-fix.md) | dispatch ready |
