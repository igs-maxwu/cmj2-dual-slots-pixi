...
| (next, perf) chore — UiButton DropShadow → inline 3-layer (chore #204 perf followup). Owner: 動畫卡卡的. Filter audit: ~26 active filters during gameplay. Removing 5 DropShadow drops to ~21, ~5ms saved per frame | [`chore/ui-button-inline-shadow-perf.md`](chore/ui-button-inline-shadow-perf.md) | dispatch ready | FillGradient 3-stop + outer dark stroke + inner gold hairline + top highlight + 4 corner accent dots + drop shadow. JP marquee style. Affects DraftScreen 4 toolbar + START BATTLE buttons | [`chore/ui-button-aesthetic-upgrade.md`](chore/ui-button-aesthetic-upgrade.md) | dispatch ready |
