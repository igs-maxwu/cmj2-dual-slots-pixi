...
| (next) chore — DraftScreen tile add SYMBOL + meta enlarge + A/B horizontal. Extract GemSymbol component (shared between SlotReel + DraftScreen). meta 9→12pt (single line). A/B vertical→horizontal (saves 36px vertical). 2-3 commits | [`chore/draft-tile-symbol-meta-buttons.md`](chore/draft-tile-symbol-meta-buttons.md) | dispatch ready |
