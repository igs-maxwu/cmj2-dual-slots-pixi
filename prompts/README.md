...
| (next) chore — Spin BlurFilter overflow fix. Per-cell rect Graphics mask added in buildCells; mask is child of cell.container so scale-aware. Confines BlurFilter strengthY=16 + gemBall.y -CELL_H slide visual within cell bounds, preventing leak above/below reel frame. Ring + arrow stay at SlotReel level so unaffected | [`chore/reel-spin-blur-overflow-mask.md`](chore/reel-spin-blur-overflow-mask.md) | dispatch ready |
