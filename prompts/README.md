...
| (next) chore — Mercenary Wild attack + damage alive-gate (mechanic spec change). Drafted hit: dmg only if matching symbolId alive. Mercenary hit: dmg only if any alive + first-alive spirit acts as proxy attacker for Wild visual (uses proxy's signature fx). Coin unchanged (player wins regardless). 3 atomic commits. **SPEC drift owner-approved 2026-05-04** | [`chore/attack-mercenary-and-alive-gate.md`](chore/attack-mercenary-and-alive-gate.md) | dispatch ready |
