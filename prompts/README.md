...
| (next) chore — Pixi v9 deprecation fix: Decorations addCornerOrnaments corner parent Graphics→Container. chore #190 audit miss — addChild on Graphics children at Decorations.ts:48,54. Wrap outer L bracket as own Graphics + corner = Container. Visual zero-change | [`chore/decorations-graphics-addChild-deprecation-fix.md`](chore/decorations-graphics-addChild-deprecation-fix.md) | dispatch ready |
| (next) chore — Attack uniform clash scale. CLASH_SCALE=1.0 const; Phase 2 lerp origAbsScale→CLASH_SCALE; Phase 3-4 use CLASH_SCALE × phase factor; Phase 5 lerp back to origAbsScale; restore preserves slot base. Back row + front row attacks same size at centre | [`chore/attack-uniform-clash-scale.md`](chore/attack-uniform-clash-scale.md) | dispatch ready |
