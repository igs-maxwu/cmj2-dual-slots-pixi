...
| (next) chore — SlotReel 轉動視覺生動化 (programmatic-first). 3 commits: vertical Y slide illusion (gemBall -CELL_H→0 per swap frame) + Pixi BlurFilter strengthY 16/22 vertical motion blur + gold light streaks per-col top/bottom edges. No new assets; future AI motion-blur asset path noted | [`chore/reel-spin-motion-vivid.md`](chore/reel-spin-motion-vivid.md) | dispatch ready |
| (running) s13-fx-03 — Free Spin retrigger 「MORE SPINS!」 ceremony. New FreeSpinRetriggerCeremony.ts (1.6s, 4 stages: rainbow halo expand → MORE SPINS! 72pt + +N ROUNDS sub → 16 LightBall radial burst from sos2-bigwin → fade). Fire-and-forget in refreshFreeSpinOverlay retrigger branch | [`sprint13/s13-fx-03-freespin-retrigger-ceremony.md`](sprint13/s13-fx-03-freespin-retrigger-ceremony.md) | dispatch ready |
