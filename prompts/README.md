...
| (next) chore — ResultScreen return button hitArea fix (Pixi 8 chore #151 pattern). Owner: 「返回 DRAFT」常點不到. Graphics+offset roundRect+eventMode=static needs explicit Rectangle hitArea. 1-line + import. ResultScreen.ts only | [`chore/result-return-btn-hitarea.md`](chore/result-return-btn-hitarea.md) | dispatch ready |
| (running) chore — AUTO continues through FreeSpin (reverses chore #162 over-aggressive stop). Owner trial: AUTO 25 + scatter trigger → AUTO stops after 1st free spin → manual click needed; expected behavior is AUTO auto-plays through free spins (industry standard, entry ceremony s13-fx-01 already provides 2.3s notice). Removes 1 line + comment update at L1831; JP/match-end/unmount stops preserved | [`chore/auto-continue-through-freespins.md`](chore/auto-continue-through-freespins.md) | dispatch ready |
