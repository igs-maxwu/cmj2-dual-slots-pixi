...
| (next) chore — Attack no-clone (move formation spirit). Replace chore #177 clone Sprite avatar pattern with directly animating cellsA[slot].container. Save+restore original x/y/scale.x/scale.y/zIndex. faceDir multiplied with origAbsScale (chore #181 0.85-1.10 base). HP bar follows spirit during attack | [`chore/attack-no-clone-move-formation-spirit.md`](chore/attack-no-clone-move-formation-spirit.md) | dispatch ready |
