...
| (running) s13-fx-02 — Streak multiplier fly-text + JP NT$ fly-in to wallet (sos2-fly-multiplier + sos2-particles trail; 2 new fx modules StreakFlyText.ts + JackpotFlyIn.ts) | [`sprint13/s13-fx-02-streak-fly-jp-fly-in.md`](sprint13/s13-fx-02-streak-fly-jp-fly-in.md) | dispatched |
| (urgent hotfix) chore — playAttackAnimations TypeError fix (chore #161 missed point — addSide reads attackerFormation.findIndex 0-8 but attackerCells only 0-4 → undefined.container crashes round loop, blocks SPIN) | [`chore/hotfix-attack-animations-active-units.md`](chore/hotfix-attack-animations-active-units.md) | dispatch ready |
