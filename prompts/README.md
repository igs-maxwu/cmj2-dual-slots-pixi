...
| (next, hotfix) chore — popDamage sparse-to-dense slot index fix. distributeDamage returns 9-elem grid slotIndex (0-8) but slotToArenaPos uses 5-elem dense (0-4) post chore #181 → fallback to slot 0 → dmg numbers appear on dead spirit. Fix: sparseToDense map at playDamageEvents boundary. **chore #181 missed audit (lesson #1 repeats)**. 1 commit | [`chore/popDamage-sparse-dense-index-fix.md`](chore/popDamage-sparse-dense-index-fix.md) | dispatch ready |
